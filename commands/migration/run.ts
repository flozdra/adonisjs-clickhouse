 ...
 * migrations in `up` direction.
 */
export default class Migrate extends MigrationsBase {
  static commandName = 'clickhouse:migration:run'
  static description = 'Migrate ClickHouse database by running pending migrations'
   ...
   * Perform dry run
   */
  @flags.boolean({ description: 'Do not run actual queries.' })
